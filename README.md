...
- Export a combined master `.dxf` (one block per part; Save As when supported, otherwise download)
...
- **Export** — below Import; saves the current layout as a master `.dxf` (each part is a DXF block with its position/rotation/mirror). Re-importing that file restores the individual parts. Save As when supported, otherwise a download.
