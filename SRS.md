...
| # | Feature | Status |
|---|---------|--------|
| 1 | Drag and drop a DXF onto the canvas | 🟢 |
| 2 | Multi-file drop; spread parts evenly | 🟢 |
| 3 | Drag a selected part (or multi-selection) to move | 🟢 |
| 4 | Rotate handle on selection | 🟢 |
| 5 | Click to select block; empty click / Escape clear | 🟢 |
| 6 | Shift+click multi-select blocks; Ctrl+click multi-select entities | 🟢 |
| 7 | Marquee (box) select blocks | 🟢 |
| 8 | Mirror selection horizontally (toolbar) | 🟢 |
| 9 | Mirror selection vertically (toolbar) | 🟢 |
| 10 | Delete selection (toolbar + Delete/Backspace, with confirm) | 🟢 |
| 11 | Pan (middle-mouse / Space+drag, grabbing cursor) and wheel zoom | 🟢 |
| 12 | Parts list selection sync with canvas (Ctrl/Cmd+click toggle; Shift+click range) | 🟢 |
| 13 | Layer / color assignment (per block or per entity) | 🟢 |
| 14 | Workspace bounding box: thin grey outline around all parts, with width×height in mm | 🟢 |
| 15 | Master DXF export (BLOCK+INSERT per part) with round-trip re-import | 🟢 |
| 16 | GitHub Pages deploy workflow | 🟢 |
| 17 | In-app help modal (info button on canvas) | 🟢 |
| 18 | Duplicate selection (toolbar offsets ~50px; Ctrl/Cmd+D places at cursor; unique `(n)` names in Parts list) | 🟢 |
| 19 | Export origin normalization (investigate LightBurn placement; shift/normalize if needed) | 🔴 |
| 20 | Undo / redo | 🔴 |
| 21 | Rename parts in the Parts list | 🔴 |
| 22 | SVG import (convert to DXF entities on import or at export) | 🔴 |
...
**Phase 3: Transformations (Move, Rotate, Mirror, Duplicate, Delete)** — 🟢
